...
10. Перед использованием любой сторонней библиотеки/фреймворка сверяться с
    актуальной документацией через **Context7** (см. раздел 13).

---

## 13. Работа с документацией библиотек (Context7)

В проекте подключён MCP-сервер **Context7** (см. `opencode.json`). Это
источник правды для актуальной документации сторонних библиотек и фреймворков,
входящих в стек (раздел 2), а также любых новых зависимостей.

### Когда обязателен Context7

Перед использованием любой сторонней библиотеки, фреймворка или их
функций/хуков/API агент **обязан** свериться с актуальной документацией через
Context7 — особенно при сомнениях в сигнатуре, существовании функции,
поведении по умолчанию или особенностях версии.

### Порядок вызова

1. `context7_resolve-library-id` — получить точный Context7-совместимый ID
   библиотеки (если ID не передан явно в формате `/org/project`).
2. `context7_query-docs` — запросить релевантную документацию/примеры по
   конкретному вопросу. Запрос формулировать предметно
   (например: «how to define nested routes with children in Vue Router»).

### Лимиты и правила

- **Не более 3 вызовов** Context7 на одну задачу — формулировать запросы
  точно и ёмко.
- Если пользователь указал версию в формате `/org/project/version` —
  использовать именно её.
- При расхождении между предположением агента и ответом Context7 приоритет
  у **Context7** (актуальная документация).

### Когда Context7 НЕ нужен

При работе с собственным кодом проекта: доменными типами (`src/types/`),
утилитами (`src/utils/`), сторами, компонентами, маршрутами — всем, что не
